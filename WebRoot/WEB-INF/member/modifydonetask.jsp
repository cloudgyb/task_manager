  ...
  <body>
  	<div class="panel panel-primary">
  		<div class="panel-heading">
			<span class="h3" style="font-family:隶书">修改任务</span>
  		...
  		<div class="panel-body">
    		...
    		<span class="text-primary">任务相关资料：&nbsp;</span><a href="downloadfile.action?downloadFileName=${requestScope.task.taffix}" class="btn btn-success">点击下载</a>
   			</c:if>
		   ...
		   <form action="member/usertasksubmit.action?uid=${sessionScope.task_user.uid}&tid=${requestScope.task.tid}" method="post" enctype="multipart/form-data">
				...
				<textarea id="ttext1" name="ttext1" rows="8" class="form-control">${requestScope.utstatus.ttext1 }</textarea>
			    <p class='text-danger'>共可以填写16380字，还可以填写<span id='wordnum'>16380</span>个字</p>
			    <hr>
			    <h5 class="text-primary">任务完成过程中的心得与体会</h5>
			    <textarea id="ttext2"  name="ttext2" rows="8" class="form-control">${requestScope.utstatus.ttext2 }</textarea>
			    <p class='text-danger'>共可以填写16380字，还可以填写<span id='wordnum1'>16380</span>个字</p>
			    <hr>
		    	<span class="text-primary">任务完成相关文档 &nbsp;</span>
		    	<input type="file" id="tfile" name="file" style="display:inline" />
		    	<span style="color:red">
		    		*可以上传任务完成后的程序或文档等相关的材料的压缩文件，格式rar,zip,7z,大小不超过20M
		    	</span><br><br>
			  	<input type="button" id="submitBtn" value="提交修改" class="btn btn-success btn-sm"/>
		  		<span style="color:red">
		    		注意：提交修改将会覆盖以前上传的文件！
		    	</span><br>
		  	</form>	
  	...
  	<script>
	  	$(function () {
			...
			$('#wordnum').html(16380-$("#ttext1").val().length);
			$('#wordnum1').html(16380-$("#ttext2").val().length);
